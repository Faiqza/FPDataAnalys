...
correlation_matrix = data[['PM2.5', 'PM10']].corr()
fig, ax = plt.subplots()
ax.scatter(correlation_matrix['PM2.5'], correlation_matrix['PM10'], s=32, alpha=0.8)
...
data_time_series = data.groupby('year')['NO2'].mean()
fig, ax = plt.subplots()
ax.plot(data_time_series.index, data_time_series.values, label='NO2', color='Blue')
...
Grafik menunjukkan adanya hubungan yang positif antara konsentrasi PM2.5 dan PM10. 
Ketika konsentrasi PM2.5 meningkat, cenderung juga terjadi peningkatan konsentrasi PM10.
 Hal ini mengindikasikan bahwa sumber polusi yang sama atau faktor-faktor yang serupa mungkin 
berkontribusi terhadap peningkatan konsentrasi partikel PM2.5 dan PM10. Misalnya, aktivitas 
pembakaran atau industri yang menghasilkan partikel-partikel tersebut.

- Conclution pertanyaan 2 :
Dari grafik, dapat diamati bahwa pada tahun 2017, stasiun yang ditunjukkan sebagai "2017-1" 
memiliki tingkat rerata NO2 yang paling tinggi dibandingkan dengan stasiun lainnya. Hal ini 
menunjukkan bahwa pada tahun 2017, stasiun tersebut mungkin mengalami tingkat polusi NO2 yang 
lebih tinggi dibandingkan dengan stasiun lainnya. Penyebabnya bisa bermacam-macam, seperti peningkatan 
aktivitas industri, transportasi, atau kebakaran. Hal ini menunjukkan bahwa tahun 2017 mungkin merupakan 
tahun dengan tingkat polusi NO2 yang lebih tinggi di wilayah yang diamati.
""")
